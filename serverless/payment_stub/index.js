...
app.post('/pay', requireApiKey, (req, res) => {
  ...
  setTimeout(() => {
    ...
    return res.json({ success: true, transactionId, amount, currency, orderId });
  }, 500);
});
...
app.post('/create-payment-intent', requireApiKey, async (req, res) => {
  ...
  if (stripeSecret) {
    try {
      ...
      return res.json({ success: true, paymentIntentId: pi.id, clientSecret: pi.client_secret });
    } catch (err) {
      console.error('Stripe create failed', err);
      return res.status(500).json({ success: false, message: 'Stripe error', error: String(err) });
    }
  }

  // Fallback: return a simulated payment intent
  const paymentIntentId = `pi_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`;
  return res.json({ success: true, paymentIntentId, clientSecret: `cs_${paymentIntentId}` });
});

app.listen(PORT, () => {
  // eslint-disable-next-line no-console
  console.log(`Payment stub listening on http://localhost:${PORT}`);
