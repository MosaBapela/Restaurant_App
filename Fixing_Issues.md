...
15. **Admin order status update not reflected on user side in realtime**
    - **Root cause:** `OrderManagementScreen` used a one-time `fetchAllOrders()` on mount — no Firestore listener. When admin updated a status it wrote to Firestore correctly, but the admin screen itself never reacted to those changes. The user-side `subscribeToUserOrders` (`onSnapshot`) was already wired, but only fired when Firestore data changed — which it did, so the user side was actually correct. The deeper fix needed was to also give the admin realtime updates.
    - **Fix (2 files):**
      1. `orderService.ts` — Added `subscribeToAllOrders(onData, onError)` using Firestore `onSnapshot` on all orders (admin view), returns `Unsubscribe` for cleanup.
      2. `OrderManagementScreen.tsx` — Replaced one-time `useEffect(() => loadOrders())` with `subscribeToAllOrders` subscription. Pull-to-refresh kept as manual fallback. Subscription cleans up on unmount.
    - **Result:** Admin updates a status → Firestore is written → both the admin list AND the user's order history update in realtime with no refresh needed.

16. **Admin food item deletion not immediately reflected on user frontend**
    - **Root cause:** Food items were loaded once at app startup via `initializeFoods` thunk (a one-time Firestore fetch). Neither `ManageFoodScreen` nor `HomeScreen` had a live Firestore listener. The admin delete flow had a post-delete re-fetch (`serviceFetchFoodItems()`) but this was a race condition — if Firestore hadn't fully propagated the delete yet, the item could reappear.
    - **Fix (3 files):**
      1. `foodService.ts` — Added `subscribeToFoodItems(onData, onError): Unsubscribe` using Firestore `onSnapshot` on the `foods` collection (ordered by name). Normalises `Timestamp` fields on each item.
      2. `ManageFoodScreen.tsx` — Replaced one-time `useEffect` fetch with `subscribeToFoodItems` subscription. Removed the racy post-delete re-fetch (`serviceFetchFoodItems()`) from `handleDeleteItem` entirely — the live listener automatically removes the item from Redux once Firestore confirms deletion. Optimistic `reduxDeleteFoodItem` dispatch retained for instant UI feedback.
      3. `HomeScreen.tsx` — Added `useEffect` with `subscribeToFoodItems` → dispatches `setFoodItems` on every Firestore change. Subscription cleans up on unmount.
    - **Result:** When an admin deletes a food item it disappears from both the admin `ManageFoodScreen` list and the user-facing `HomeScreen` in realtime, with no manual refresh required.
